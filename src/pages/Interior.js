...
// Define interior product/service data with placeholder images (data remains the same)
...
function Interior() {
  ...
  }, [interiorProducts]); // Dependency on products array
  // --- End Animation Logic ---
    ...
    <main className="product-page-container" aria-labelledby="page-title">
      ...
      <header className="product-page-header animate-slide-down">
        <div className="header-content">
           <h1 id="page-title">Eco Interior Design</h1>
           <p className="page-subtitle">Sustainable style for conscious living.</p>
        </div>
        <button
          ...
          {interiorProducts.map((product, index) => (
            ...
            >
              <ProductCard
                ...
                // isService={product.isService} // Pass if ProductCard handles it
              />
            </div>
          ))}
        </div>
